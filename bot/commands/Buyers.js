...
/**
 * Command to get a list of buyers from warframe.market for a given
 * item. Call with "/buyers # mod name" where # is the number of
...
class Buyers extends Command
{
	...
	{
		// Helptext values
		let desc  = `Get a list of Warframe.market buyers for an item`;
		let usage = `${settings.prefix}buyers [quanitity] <item name>`;
		let help  = `Examples:
	${settings.prefix}buyers transient fortitude
	${settings.prefix}buyers 10 blind rage

The buyers command will get and return a list of users on Warframe.market that are online in-game and buying the item you are looking up. The number of listings to be requested is optional. If omitted, up to 5 listings will be returned. A maximum of 10 listings can be retrieved at any time to save on message space.
...
When searching for prime blueprint sets, just do the prime name + set. For example:
	...
	${settings.prefix}buyers ember prime set`;

		// Activation command regex
		var command = /^buyers (?:(\d{1,2}) )?([\w'’\(\)][\w '’\&\(\)\-]+[\w'’\(\)])$/;
		...
		var action = (message, resolve, reject) =>
		{
			 ...
			 * Get a list of item listings and parse online buyers
			 * from the data
			...
			{
				result = JSON.parse(data)["response"]["buy"];
				...
				if (validResults.length < 1)
				{
					message.channel.sendCode("css",
						`No online offers available for [${name}] buyers.\n` +
