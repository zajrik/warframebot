...
export default class WfBot extends Bot
{
	...
	public constructor(botOptions: BotOptions)
	{
		...
		this.timers.add(new Timer(this, 'events', 2 * 60, () => this.eventLoader.loadEvents()));
		this.timers.add(new Timer(this, 'items', 5 * 60 * 60, () => this.itemLoader.reloadItems()));
		this.timers.add(new Timer(this, 'notify', 60, () => this.notifier.checkEvents()));
		this.timers.add(new Timer(this, 'pruneNotifications', 5 * 60, () => this.notifier.checkExpired()));
	}
}
